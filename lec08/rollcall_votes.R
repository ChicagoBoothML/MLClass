...
hist(loadings[,2], main="", xlab="2nd Principle Component Vote-Loadings",
     col=8, border=grey(.9)) # there is dichotomy

plot(votepc[,2]~legis$party) # dichotomy cannot be explained by party membership
...
attendance<-rowSums(votes==0)

plot(attendance~votepc[,2])
